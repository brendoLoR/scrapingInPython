...
class findInUrl:
    ...
    def find(self):
        ...
        for tag in tags:
            published_date = tag.find(self.tag_name['published_date'], class_=self.class_name['published_date']).text.replace('\r', '').replace('\n', '')

            if (self.key_word in published_date):
                apply_link = tag.find(self.tag_name['apply_link']).a['href']
                company_name = tag.find(self.tag_name['company_name'], class_=self.class_name['company_name']).text.replace(' ', '').replace('\r','').replace('\n', '')
                skills = tag.find(self.tag_name['skills'], class_=self.class_name['skills']).text.replace(' ', '').replace('\r', '').replace('\n', '')
